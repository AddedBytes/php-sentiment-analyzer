...
class Analyzer
{
    ...
    private array|null $lexicon;
    private SentiText $current_sentitext;
    private readonly string $emoji_lexicon;
    private array|null $emojis;
    ...
    */
    ...
    private function IsKindOf($firstWord, $secondWord): bool
    {
        return strtolower((string) $firstWord) === "kind" && strtolower((string) $secondWord) === "of";
    }
    ...
    private function getValenceFromLexicon($word): mixed
    {
        return $this->lexicon[strtolower((string) $word)];
    }

    private function getTargetWordFromContext($wordInContext): mixed
    {
        return $wordInContext[count($wordInContext) - 1];
    }

    /*
        Gets the preceding two words to check for emphasis
    */
    private function getWordInContext($wordList, $currentWordPosition): array
    {
        $precedingWordList = [];
    ...
    */
    public function getSentiment($text): array
    {
        ...
        for ($i = 0; $i <= count($words_and_emoticons) - 1; $i++) {
            ...
            if ($this->IsInLexicon($wordBeingTested)) {
                ...
                if ($words_and_emoticons[$i] != "kind" && $words_and_emoticons[$i] != "of") {
    ...
    private function str_split_unicode($str, $l = 0): array|false
    {
        if ($l > 0) {
            $ret = [];
    ...
    private function applyValenceCapsBoost($targetWord, $valence): float
    {
    ...
    /*
        ...
        valence
     */
    private function boosterScaleAdjustment($word, $valence): float
    {
        $scalar = 0.0;
    ...
    private function dampendBoosterScalerByPosition($booster, $position): mixed
    {
        if ($booster === 0) {
            return $booster;
        }
    ...
    private function adjustBoosterSentiment($wordInContext, $valence): mixed
    {
    ...
    private function modifyValenceBasedOnContext($wordInContext, $valence): mixed
    {
        ...
        return $this->_least_check($wordInContext, $valence);
    }

    public function _least_check($wordInContext, $valence): mixed
    {
        // check for negation case using "least"
        //if the previous word is least"
        if (strtolower((string) $wordInContext[2]) == "least") {
            //but not "at least {word}" "very least {word}"
            if (strtolower((string) $wordInContext[1]) != "at" && strtolower((string) $wordInContext[1]) != "very") {
    ...
    {
        $one_zero = sprintf("%s %s", $wordInContext[2], $wordInContext[3]);

        $two_one_zero = sprintf("%s %s %s", $wordInContext[1], $wordInContext[2], $wordInContext[3]);

        $two_one = sprintf("%s %s", $wordInContext[1], $wordInContext[2]);

        $three_two_one = sprintf("%s %s %s", $wordInContext[0], $wordInContext[1], $wordInContext[2]);

        $three_two = sprintf("%s %s", $wordInContext[0], $wordInContext[1]);

        $zero_one = sprintf("%s %s", $wordInContext[3], $wordInContext[2]);

        $zero_one_two = sprintf("%s %s %s", $wordInContext[3], $wordInContext[2], $wordInContext[1]);

        $sequences = [$one_zero, $two_one_zero, $two_one, $three_two_one, $three_two];
        ...
        foreach ($sequences as $seq) {
            ...
            */
            ...
            if ($this->IsBoosterWord($three_two) || $this->IsBoosterWord($two_one)) {
    ...
    public function _never_check($wordInContext, $valance): float|int
    {
        //If the sentiment word is preceded by never so/this we apply a modifier
        $neverModifier = 0;
        if ($wordInContext[0] == "never") {
            $neverModifier = 1.25;
        }
        if ($wordInContext[1] == "never") {
            $neverModifier = 1.5;
        }
        if ($wordInContext[1] == "so" || $wordInContext[2] == "so" || $wordInContext[1] == "this" || $wordInContext[2] == "this") {
    ...
    public function _sentiment_laden_idioms_check($valence, $senti_text_lower): mixed
    {
        # Future Work
        ...
        if ($idioms_valences !== []) {
            // $valence = ( array_sum( explode( ',', $idioms_valences ) ) / (float)strlen($idioms_valences));
        }

        return $valence;
    }

    public function _punctuation_emphasis($text): float|int
    {
    ...
    public function score_valence($sentiments, $text): array
    {
        if ($sentiments) {
            ...
            $punct_emph_amplifier = $this->_punctuation_emphasis($text);
            if ($sum_s > 0) {
